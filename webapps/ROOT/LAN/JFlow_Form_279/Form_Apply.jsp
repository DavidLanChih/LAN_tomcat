...
//公式:(抓取表單欄位總數-非供應商欄位數)=表單櫃號欄位總數
TransferNo=FieldCount-13; 
//----------釋放-----------
closeConn(Data,Conn);
%>
...
document.getElementById("FD_Data3").value=d1;
var Obj1 = document.getElementById("FD_Data3");
    Obj1.setAttribute("readOnly",'true');

//將IP寫入第三欄位(鎖住變更)
document.getElementById('FD_Data4').value='<%=ipAddress %>';
var Obj2 = document.getElementById("FD_Data4");
...
function doApply()
{
	...
	var APArr=Frm.FD_Data1.value.split(' ');
		...
		SurverArr_1=Frm.FD_Data<%=TransferNo+6%>.value.split(':');		
		S1=SurverArr_1[1].substring(0,5).trim();
	if(AP==S1)
	{
		alert('第一關：主管不得審自己!');
		Frm.tmp_FD_Data<%=TransferNo+6%>.focus();
		return false;
	}
	...
	//第一關
	if(document.getElementById('tmp_FD_Data<%=TransferNo+6%>').value=='')
	{
		document.getElementById('FD_Data<%=TransferNo+10%>').innerText='';
		alert('請選擇第一關審核人員!');
		Frm.tmp_FD_Data<%=TransferNo+5%>.focus();
		return false;
	}
	else
	{		  
		var SurverArr=Frm.FD_Data<%=TransferNo+6%>.value.split(':');	  
		var ID=SurverArr[1].substring(0,5).trim(); 		  		  
		if(ID!='' && SurverArr[1].substring(0,5).trim().length==4)
		{
			UID="cy"+Frm.tmp_FD_Data<%=TransferNo+6%>.value.trim();
		}
		if(ID!='' && SurverArr[1].substring(0,5).trim().length==5)
		{
			UID="c"+Frm.tmp_FD_Data<%=TransferNo+6%>.value.trim()+1;
		}		
		var Field1 = 'FD_Data<%=TransferNo+10%>'; 
		list.push(UID);
		listfid.push(Field1);	 
	}
	...
	//第二關
	if(document.getElementById('tmp_FD_Data<%=TransferNo+7%>').value=='')
	{
		document.getElementById('FD_Data<%=TransferNo+11%>').innerText='';
		alert('請選擇第二關審核人員!');
	...
	{
		var SurverArr=Frm.FD_Data<%=TransferNo+7%>.value.split(':');	  
		var ID=SurverArr[1].substring(0,5).trim(); 		  
		  
		if(ID!='' && SurverArr[1].substring(0,5).trim().length==4)
		{
			UID="cy"+Frm.tmp_FD_Data<%=TransferNo+7%>.value.trim();
		}
		if(ID!='' && SurverArr[1].substring(0,5).trim().length==5)
		{
			UID="c"+Frm.tmp_FD_Data<%=TransferNo+7%>.value.trim()+1;
		}
		
		var Field1 = 'FD_Data<%=TransferNo+11%>'; 
		list.push(UID);
		listfid.push(Field1);
	}
	 
	//第三關
	if(document.getElementById('tmp_FD_Data<%=TransferNo+8%>').value=='')
	{
		document.getElementById('FD_Data<%=TransferNo+12%>').innerText='';
		alert('請選擇第三關審核人員!');
		Frm.tmp_FD_Data<%=TransferNo+8%>.focus();
		return false;
	}
	else(document.getElementById('tmp_FD_Data<%=TransferNo+8%>').value!='')
	{
		var SurverArr=Frm.FD_Data<%=TransferNo+8%>.value.split(':');
		var ID=SurverArr[1].substring(0,5).trim();  
		...
		var Field1 = 'FD_Data<%=TransferNo+12%>'; 
		list.push(UID);
		listfid.push(Field1);
	}
	  
	//第四關
	if(document.getElementById('tmp_FD_Data<%=TransferNo+9%>').value=='')
	{
		document.getElementById('FD_Data<%=TransferNo+13%>').innerText='';
		alert('請選擇第四關審核人員!');
		Frm.tmp_FD_Data<%=TransferNo+9%>.focus();
		return false;
	}
	else
	{
		UID=document.getElementById('tmp_FD_Data<%=TransferNo+9%>').value.trim()+1;
		var Field1 = 'FD_Data<%=TransferNo+13%>'; 
		list.push(UID);
		listfid.push(Field1);
	}
...
{   
    ...
    {
		var SP='';
		S1=document.getElementById('FD_Data<%=TransferNo+10%>').innerText;
		S2=document.getElementById('FD_Data<%=TransferNo+11%>').innerText;
		S3=document.getElementById('FD_Data<%=TransferNo+12%>').innerText;  //101; //商務資訊課主管OrgId
		S4=document.getElementById('FD_Data<%=TransferNo+13%>').innerText;  //241; //商務資訊課OrgId
