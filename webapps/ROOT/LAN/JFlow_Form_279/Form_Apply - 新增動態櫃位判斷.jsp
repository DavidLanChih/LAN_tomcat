...
function doApply()
{
	...
	var APArr=Frm.FD_Data1.value.split(' ');
		...
		SurverArr_1=Frm.FD_Data<%=TransferNo+5%>.value.split(':');		
	...
	//第一關
	if(document.getElementById('tmp_FD_Data<%=TransferNo+5%>').value=='')
	{
		document.getElementById('FD_Data<%=TransferNo+9%>').innerText='';
		alert('請選擇第一關審核人員!');
		Frm.tmp_FD_Data<%=TransferNo+5%>.focus();
		return false;
	}
	...
	{		  
		var SurverArr=Frm.FD_Data<%=TransferNo+5%>.value.split(':');	  
		var ID=SurverArr[1].substring(0,5).trim(); 		  		  
		if(ID!='' && SurverArr[1].substring(0,5).trim().length==4)
		{
			UID="cy"+Frm.tmp_FD_Data<%=TransferNo+5%>.value.trim();
		}
		if(ID!='' && SurverArr[1].substring(0,5).trim().length==5)
		{
			UID="c"+Frm.tmp_FD_Data<%=TransferNo+5%>.value.trim()+1;
		}		
		var Field1 = 'FD_Data<%=TransferNo+9%>'; 
		list.push(UID);
		listfid.push(Field1);	 
	}
	...
	if(document.getElementById('tmp_FD_Data<%=TransferNo+7%>').value=='')
	{
		...
		alert('請選擇第三關審核人員!');
		Frm.tmp_FD_Data<%=TransferNo+7%>.focus();
		return false;
	}
	else(document.getElementById('tmp_FD_Data<%=TransferNo+7%>').value!='')
	{
	...
	{
		UID=document.getElementById('tmp_FD_Data<%=TransferNo+8%>').value.trim()+1;
...
{   
    ...
    {
		var SP='';
		S1=document.getElementById('FD_Data<%=TransferNo+9%>').innerText;
		S2=document.getElementById('FD_Data<%=TransferNo+10%>').innerText;
		S3=document.getElementById('FD_Data<%=TransferNo+11%>').innerText;  //101; //商務資訊課主管OrgId
		S4=document.getElementById('FD_Data<%=TransferNo+12%>').innerText;  //241; //商務資訊課OrgId
