...
<%@ include file="/Modules/ServiceFormCheck5/Util_Rule1.jsp" %>
...
//Form Var 090502加入此段程式，可在審核中帶入欄位表單變數
String value="";
...
//權限更改(王20190513)(申請內容)
SqlStr="select * from flow_formdata where FD_RecId="+Data.toSql(FD_RecId)+" limit 1";
...
out.flush();
    ...
    Grid.Init();
    if(!OP.equals("Check")) Grid.isCatchBuffer(false);//090502表單審核後如要再編輯，需加此行
    ...
    if(FormFieldCount>0)
    {
        ...
        {        
              ...
              {                           
                    ...
                    {
                    	//090430如遇到CurrStage關卡&&FieldCount欄位的話，把readonly拿掉，變成可以修改
                    	if((CurrStage==4 && FieldCount==31) || (CurrStage==9 && FieldCount==60) || (CurrStage==9 && FieldCount==68) || (CurrStage==9 && FieldCount==69))
                    	{PutContent+="<input type=text readonly value='"+DataRs.getString("FD_Data"+FieldCount)+"' name='FD_Data"+FieldCount+"' id='FD_Data"+FieldCount+"' onclick='calendar(this);' maxlength=10 size=8>";
                        ...
                        if(Rs.next())
                        {
                            ...
                            switch(Integer.parseInt(F_Type))
                            {
                                ...
                                case 2:  //checkbox
                                    ...
                                    break;
                                case 3:  //select
                                	if((CurrStage==4 && FieldCount==30) || (CurrStage==5 && FieldCount==47) || 
										(CurrStage==5 && FieldCount==48) || (CurrStage==5 && FieldCount==52) || 
                                		(CurrStage==6 && FieldCount==54) || (CurrStage==8 && FieldCount==57) ||
                                		(CurrStage==9 && FieldCount==59) || (CurrStage==9 && FieldCount==70) || 
                                		(CurrStage==9 && FieldCount==71) || (CurrStage==9 && FieldCount==72) || 
                                		(CurrStage==9 && FieldCount==73) || (CurrStage==9 && FieldCount==74))
                                	{
                                  	...
                                  	{                                  	
	                                    ...
	                                    MStr.append("<select size=1  disabled name='tmp_FD_Data"+FieldCount+"' id='tmp_FD_Data"+FieldCount+"' onfocus='disabledSelect()' "+EventScriptStr+">");
	                                    MStr.append("<option value=''>請選擇...</option>");
	                                    if(MustKeying.equals("1")) {ScriptStr+=" if(!checkSelecData('FD_Data"+FieldCount+"')) {alert('請選擇"+FF_Name+"!');return false;}\n";}
										if(!DataRs.getString("FD_Data"+FieldCount).equals(""))
										{
...
function setAgent1()
{
	...
	{
		var USID=trim(Frm.FD_Data1.value.split(",")[0]);		
		window.open('/cy/getEventRel.jsp?Field=FD_Data32&USID='+USID,'A','width=200,height=200,scrollbar=1');                                           
	}
}	

function setAgent2()
{
	  window.open('/cy/getDeptRel.jsp?Field=FD_Data31','A','width=200,height=200,scrollbar=1');    
}	
