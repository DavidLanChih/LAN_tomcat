...
{				
	...
	TransferGroup=(FieldCount-13)/2;                          
	out.print("取得櫃號組數，共"+TransferGroup+"組！");
		...
		{				
			try
			{
				...
				if(!Data1.equals(""))                                                 
				{
					...
					{
						u++;
						Field5=String.valueOf((t*2)+6);	                                                         //原供應商FD_Data欄位
						Field6=String.valueOf((t*2)+7);	                                                         //轉移供應商FD_Data欄位
					...
					{							
						...
						out.print("表單:"+FDId+"標記或mail承辦人失敗，返回至資料庫標記前狀態！");					
					}						
					finally 
					{									
			...
			{							
				...
				out.print("表單:"+FDId+"更新失敗，返回至更新前狀態！");					
			}						
			finally 
			{									
...
{							
	...
	out.print("櫃號轉移失敗！錯誤原因:"+catch_ex+"");		
}						
...
if(catch_e!=""||catch_ex!=""||catch_d!="")
{
	...
	transport.sendMessage(message, message.getAllRecipients());
	transport.close();	
	out.print("已將錯誤訊息Email至商務資訊課負責人員!");
}
else
{
