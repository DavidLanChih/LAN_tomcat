		...
		{				
			try
			{
				...
				if(!Data1.equals(""))                                                 
				{
					...
					{
						SqlStrS+="update f set BillNo"+s+"='"+FDId+"' where FR_DataId="+FDId;
					}
...
{							
	...
	%><script>alert("Transfer.jsp 錯誤 原因:<%=catch_ex %>");</script><%			
}						
...
if(catch_e!=""||catch_ex!="")
{
	fw.write("Transfer.jsp 表單:"+FDId+" 錯誤訊息:"+catch_e+catch_ex+"\r\n");
}
else
{
	fw.write(Record_All);		
...
if(catch_e!=""||catch_ex!="")
{
	SqlStr="insert into Flow_Log (L_FormName,L_ActiveType,L_FormMemo) values ('櫃號轉移表單-JFlow_Form_279','批次轉入','錯誤: Transfer.jsp  FR_DataId="+catch_e+catch_ex+"')";
	Data.ExecUpdateSql(Conn,SqlStr);
