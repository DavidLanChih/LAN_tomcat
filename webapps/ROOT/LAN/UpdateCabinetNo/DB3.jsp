			...
			//-----確認表單沒有更新過的標記-----
				if(DRs.getString("BillNo1").equals("")&&DRs.getString("BillNo2").equals(""))                          
				{
					...
					if(!Data1.equals(""))                                                 
					{
						try
						{
							...
							for(int t=0;t<2;t++)              
							{
								...
								if(!Data2.equals("")) 
								{
									...
									//CHANGE_GOODS資料更新
									SqlStrEC1="update CHANGE_GOODS set COUNTER_ID='11081'where COUNTER_ID='11069'";
									stmt.executeUpdate(SqlStrEC1);
											
									//GOODS_COUNTER資料更新
									SqlStrEC2="update GOODS_COUNTER set COUNTER_ID='11081'where COUNTER_ID='11069'";
									stmt.executeUpdate(SqlStrEC2);
											
									//GOODS資料更新
									SqlStrEC3="update GOODS set SUPPLIER_ID='11081'where SUPPLIER_ID='11069'";
									stmt.executeUpdate(SqlStrEC3);
											
									//GOODS_EC_PRICE資料更新
									SqlStrEC4="update GOODS_EC_PRICE set COUNTER_ID='11081'where COUNTER_ID='11069'"; 
						...
						stmt4.executeUpdate("insert into Flow_Log (L_FormName,L_ActiveType,L_FormMemo) values ('櫃號轉移表單-JFlow_Form_279','批次轉入','第四次結束...')");			
...
{							
	catch_e=e.getMessage();	
	%><script>alert("表單:<%=FDId%>標記失敗，返回至標記前狀態！");</script><%	
}						
%>