    ...
    if(FormFieldCount>0)
    {
        ...
        {        
              ...
              {  
                    ...
                    {         
                        if(FieldCount==15)
                        {PutContent+="<input type=text name='FD_Data"+FieldCount+"' id='FD_Data"+FieldCount+"' onclick=IP() readonly value='"+(UI.req("FD_Data"+FieldCount).equals("")?FF_Value:UI.req("FD_Data"+FieldCount))+"' size='"+Size+"' maxlength='"+Size+"' "+EventScriptStr+">";}
                        else 
...
function IP()
{		
		window.open('/cy/IP.jsp','IP','width=550,height=200,scrollbar=1');		
}
document.getElementById('FD_Data10').value='<%=TD%>';
//Step 1
function doApply()
{
    if(DataCheck())
