    ...
    if(FormFieldCount>0)
    {
        ...
        {        
              ...
              {  
                    ...
                    {         
                        if(FieldCount==2)
                        {PutContent+="<input type=text name='FD_Data"+FieldCount+"' id='FD_Data"+FieldCount+"' onclick=SetCountpos() readonly value='"+(UI.req("FD_Data"+FieldCount).equals("")?FF_Value:UI.req("FD_Data"+FieldCount))+"' size='"+Size+"' maxlength='"+Size+"' "+EventScriptStr+">";}
...
function SetCountpos()
{	
		var STRNO = document.getElementById('FD_Data3').value.substring(0,5).trim();
		
		/*if (STRNO != '')
		{*/
			window.open('/Form/getPosmaic.jsp?FieNam=FD_Data2&STRNO='+STRNO,'隨便一個名字','width=300,height=300,scrollbar=1');
		/*}
		else
		{
			alert('請先選擇櫃位');
