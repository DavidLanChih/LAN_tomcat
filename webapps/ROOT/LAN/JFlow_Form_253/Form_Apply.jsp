...
String RI_SendMailToSurver="",Curr_Stage="",DeptValue="",PathSurvey="", x1="", x2="" ;
...
if(FI_Rule.equals("0"))
{
    ...
    {        
        Grid.AddRow("",2);
        Grid.AddCol("<font color=blue>表單說明:</font>","nowrap width=80 valign=top nowrap");
        Grid.AddCol("<font color=purple>"+FI_Descript.replaceAll("\n","<br>")+"</font>","colspan=2 valign=top nowrap");
    }
    if(FormFieldCount>0)
    {
        FieldCount=0;        
        SqlStr="select * from flow_formfieldinfo  where FF_FormRecId=" + FI_RecId + " limit 1";     //從資料庫的flow_formfieldinfo表單，尋找FF_FormRecId欄位=253(此表單的代號)的欄位資料
        MRs=Data.getSmt(Conn,SqlStr);
        for(int i=0;i<FormFieldCount;i++)
        {        
              ...
              if(MRs.next())
              {  
                    //x1=MRs.getString("FF_F"+3+"_Name");                                           //取得資料庫的此欄位的資料字串(表格名稱)
					//x2="<font color=red>"+x1+"</font>";                                           //將此表格名稱改為紅色
					//FF_Name=FF_Name_View=x2;                                                      //將此表格的名稱存在FF_Name以及FF_Name_View (可含html語法)
					//FF_Name=x2;                                                                   //會沒有呈現效果(主要是靠FF_Name_View)
					//FF_Name=FF_Name_View=MRs.getString("FF_F"+(i+1)+"_Name");                     //原始未修改之原始碼(將所有表格名稱存在FF_Name以及FF_Name_View，並呈現出來)
					
					if(i!=2&&i!=3&&i!=8)                                                            //(去除第3、4、5的表格名稱)
					{
						FF_Name=FF_Name_View=MRs.getString("FF_F"+(i+1)+"_Name"); 
					}
					if(FieldCount==2||FieldCount==3||FieldCount==8)                                 //將第3、4、5的表格名稱顏色改成後紅色
					{
						FF_Name_View="<font color=red>"+MRs.getString("FF_F"+(FieldCount+1)+"_Name")+"</font>";
					}
                    
					FF_TypeArr=MRs.getString("FF_F"+(i+1)+"_Type").split(":");         
					//out.println(FF_TypeArr);
                    FF_Type=FF_TypeArr[0];
					//out.println(FF_Type);
                    PropertyArr=(MRs.getString("FF_F"+(i+1)+"_Property")+";;;;").split(";",8);
					//out.println(PropertyArr);
                    ColGroup=PropertyArr[0];Size=PropertyArr[1];MustKeying=PropertyArr[2];
					//out.println(MustKeying);
                    FF_View=PropertyArr[4];
					//out.println(PutContent);
                    ...
                    {   
						if(FieldCount==4)
                        {PutContent+="<input type=text name='FD_Data"+FieldCount+"' id='FD_Data"+FieldCount+"' onclick=SetCount() readonly value='"+(UI.req("FD_Data"+FieldCount).equals("")?FF_Value:UI.req("FD_Data"+FieldCount))+"' size='"+Size+"' maxlength='"+Size+"' "+EventScriptStr+">";}
                        else
...
<script src='/Modules/JEIPKernel/jquery-3.6.0.min.js'></script>
<script>
//Step 1
function doApply()
{
	var list = [];                                                   //宣告list為陣列
	var listfid = [];                                                //宣告listfid為陣列
	var UID="";                                                      //宣告UID為空值
	 //第一關審核人員
  if(document.getElementById('tmp_FD_Data14').value=='')             //若第一關下拉式選單無值
  {
     document.getElementById('FD_Data17').innerText='';              //則儲存第一關值的隱藏欄位則為無值
     alert('請選擇第一關審核人員!');                                 //彈跳警告視窗(請選擇第一關審核人員!)
	 Frm.tmp_FD_Data14.focus();                                      //焦點鎖定在第一關下拉式選單
     return false;                                                   //此程式結束
  }
  else                                                               //若第一關下拉式選單有值
  {
	     var SurverArr=Frm.FD_Data14.value.split(':');               //將第一關下拉式選單的值依照:切割成陣列，並存在SurverArr

		  var ID=SurverArr[1].substring(0,5).trim();                 //將第一關下拉式選單值的陣列的第二個位置取第一個字至第五個字，且左右去除空白，最後存在ID
		  if(ID!='' && SurverArr[1].substring(0,5).trim().length==4) //若ID不為空值且字串有4個字
		  {
			   UID="cy"+Frm.tmp_FD_Data14.value.trim() ;             //則執行:將第一關下拉式選單的值左右去除空白後，最左邊加上cy，然後存在UID
		  }
		 if(ID!='' && SurverArr[1].substring(0,5).trim().length==5)  //若ID不為空值且字串有5個字
		 {
			  UID="c"+Frm.tmp_FD_Data14.value.trim()+1;              //則執行:將第一關下拉式選單的值左右去除空白後，最左邊加上c，最右邊加上1，然後存在UID
		 }
		
		 var Field1 = 'FD_Data17';                                   //將第一關值存在隱藏欄位的值存在Field1
		list.push(UID);                                              //在list陣列最尾巴，加入一個陣列值(工號或部門代號)
	
		listfid.push(Field1);                                        //將listfid陣列最尾巴，加入一個陣列值(第一關的值)
		if(ID==document.getElementById('FD_Data1').value.substring(0,5).trim())   //如果ID等於第一個欄位的值(第一個字至第五個字，然後左右去除空白)
      	 {
           alert('主管不得審自己!');                                              //則跳出警告(主管不得審自己!)
		   document.getElementById('tmp_FD_Data14').focus();                      //焦點鎖定在第14欄位下拉式選單
           Frm.tmp_FD_Data14.focus();                                             //焦點鎖定在第14欄位下拉式選單
           return false;                                                          //此程式結束
         }
		
  }	
	 //第二關審核人員
  if(document.getElementById('tmp_FD_Data16').value=='')             //若第二關下拉式選單無值
  {
     document.getElementById('FD_Data18').innerText='';              //則儲存第一關值的隱藏欄位則為無值
     alert('請選擇第一關審核人員!');                                 //彈跳警告視窗(請選擇第一關審核人員!)
	 Frm.tmp_FD_Data15.focus();                                      //焦點鎖定在第一關下拉式選單
     return false;                                                   //此程式結束
  }
  else                                                               //若第二關下拉式選單有值
  {
	     var SurverArr=Frm.FD_Data16.value.split(':');               //將第二關下拉式選單的值依照:切割成陣列，並存在SurverArr

		  var ID=SurverArr[1].substring(0,5).trim();                 //將第二關下拉式選單值的陣列的第二個位置取第一個字至第五個字，且左右去除空白，最後存在ID
		  if(ID!='' && SurverArr[1].substring(0,5).trim().length==4) //若ID不為空值且字串有4個字
		  {
			   UID="cy"+Frm.tmp_FD_Data16.value.trim() ;             //則執行:將第一關下拉式選單的值左右去除空白後，最左邊加上cy，然後存在UID
		  }
		 if(ID!='' && SurverArr[1].substring(0,5).trim().length==5)  //若ID不為空值且字串有5個字
		 {
			  UID="c"+Frm.tmp_FD_Data16.value.trim()+1;              //則執行:將第一關下拉式選單的值左右去除空白後，最左邊加上c，最右邊加上1，然後存在UID
		 }
		
		 var Field1 = 'FD_Data18';                                   //將第二關值存在隱藏欄位的值存在Field1
		list.push(UID);                                              //在list陣列最尾巴，加入一個陣列值(工號或部門代號)
   ...
   document.getElementById('FrmRule').src ='/Form/getOrgId2.jsp?UID='+list+'&Field1='+listfid+'';  //iframe的連結位置為/Form/getOrgId2.jsp，且輸入值UID=工號或部門代號以及Field1=第一關的值
   setTimeout(function () {                                                       //避免沒有讀到程式，做一個延緩讀取
	   
		if(DataCheck())                                                           //如果有DataCheck()，則執行
		{
				Frm.action='parseRule.jsp';                                       //表格送出到parseRule.jsp 廠商設計
				Frm.target='FrmRule';                                             //廠商設計                                           
				Frm.submit();
		}
	}, 100);
}
...
function SetCount()
{
		...
		window.open('/cy/getCountOpt123.jsp?Field=FD_Data4&Field1=FD_Data5&Field2=tmp_FD_Data6&Field3=FD_Data6&Field4=tmp_FD_Data7&Field5=FD_Data7&USID='+USID,'A','width=500,height=200,scrollbar=1');
...
{   
    ...
    if(confirm('確定送出此表單嗎?'))     
    {
	/* if(typeof(afterCheckData)=='function')
    	{
        		var val=eval('afterCheckData();');   
        		if(!val) return val;
    	}*/
		
		   var SP='';
		   S1=document.getElementById('FD_Data17').innerText;//第一關審核人員
		   S2=document.getElementById('FD_Data18').innerText;//第二關審核人員
		  // S3=188;//促銷課主管OrgId
