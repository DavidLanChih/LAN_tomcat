...
if(!FI_RecId.equals("")) 
{
	SqlStr_Form=" and (flow_form_rulestage.FR_FormId="+Data.toSql(FI_RecId)+" or flow_form_rulestage.FR_FormId='231')";
}
else                                                                                                       //如果有表單送出，
{SqlStr_Form=" and (flow_form_rulestage.FR_FormId='230' or flow_form_rulestage.FR_FormId='231')";}         //SQL語法(表單模組ID為230或231)，儲存至字串SqlStr_Form
    ...
    {  
        ...
        if(!days[i].equals("")) 
        {
			...
			SqlStr="select flow_form_rulestage.FR_RecId,flow_form_rulestage.FR_FormId,flow_form_rulestage.FR_FinishState,flow_form_rulestage.FR_OrgId,flow_form_rulestage.FR_DataId,"+
			       "flow_forminfo.FI_Name,flow_formdata.FD_Data4,flow_formdata.FD_Data5,flow_formdata.FD_Data6,flow_formdata.FD_Data7,Org.OrgName	"+ //行事曆擷取資料庫的內容
				   ...
				   "and CONVERT(varchar(10), CONVERT(datetime, flow_formdata.FD_Data5, 111), 111) <='"+strdate+"' "+        //篩選條件:小於等於選擇日期
			       "and CONVERT(varchar(10), CONVERT(datetime, flow_formdata.FD_Data5, 111), 111) >='"+strdate+"' "+        //篩選條件:大於等於選擇日期
            ...
            while(Rs.next())
            {
				if(Rs.getString("FR_FormId").equals("230"))            //若模組ID為230，則執行
				{
					if(Rs.getString("FR_FinishState").equals("0"))     //若為申請中，則執行
					{
						DayContent.append("<div><img src=/images/edit.gif border=0><a style='color:pink;' href='javascript:ProcEditEvent("+Rs.getString("FR_RecId")+","+Rs.getString("FR_FormId")+","+Rs.getString("FR_DataId")+","+Rs.getString("FR_OrgId")+")'>"+Rs.getString("FD_Data4")+"-"+Rs.getString("FD_Data5")+"  "+Rs.getString("FD_Data6")+"</a></div>");
					}                                                  //若表單狀態為申請中，則以此方式呈現於行事曆內容
				   else                                                //若非為申請中，則執行
				   {
					   DayContent.append("<div><img src=/images/edit.gif border=0><a style='color:green;' href='javascript:ProcEditEvent("+Rs.getString("FR_RecId")+","+Rs.getString("FR_FormId")+","+Rs.getString("FR_DataId")+","+Rs.getString("FR_OrgId")+")'>"+Rs.getString("FD_Data4")+"-"+Rs.getString("FD_Data5")+"  "+Rs.getString("FD_Data6")+"</a></div>");
				   }                                                   //若表單狀態為申請中，則以此方式呈現於行事曆內容
				}
				else                                                   //若模組ID非為230，則執行
				{
					if(Rs.getString("FR_FinishState").equals("0"))     //若為申請中，則執行
					{
						DayContent.append("<div><img src=/images/edit.gif border=0><a style='background-color:yellow; font-size:15px;' href='javascript:ProcEditEvent("+Rs.getString("FR_RecId")+","+Rs.getString("FR_FormId")+","+Rs.getString("FR_DataId")+","+Rs.getString("FR_OrgId")+")'>"+Rs.getString("FD_Data4")+"-"+Rs.getString("FD_Data5")+"  "+Rs.getString("FD_Data7")+"</a></div>");
					}                                                  //若表單狀態為申請中，則以此方式呈現於行事曆內容
				   else                                                //若非為申請中，則執行
				   {
					   DayContent.append("<div><img src=/images/edit.gif border=0><a style='color:blue; font-size:15px;' href='javascript:ProcEditEvent("+Rs.getString("FR_RecId")+","+Rs.getString("FR_FormId")+","+Rs.getString("FR_DataId")+","+Rs.getString("FR_OrgId")+")'>"+Rs.getString("FD_Data4")+"-"+Rs.getString("FD_Data5")+"  "+Rs.getString("FD_Data7")+"</a></div>");
				   }                                                   //若表單狀態非為申請中，則以此方式呈現於行事曆內容
				}
            }
            Rs.close();Rs=null;
