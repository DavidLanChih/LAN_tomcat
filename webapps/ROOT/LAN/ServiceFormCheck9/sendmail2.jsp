...
message.setFrom(new InternetAddress(from));                  		     //設定mail的來源信箱物件
message.addRecipient(Message.RecipientType.TO, new InternetAddress(to)); //設定mail的接收者信箱物件
message.addRecipient(Message.RecipientType.CC, new InternetAddress(CC1));
message.setSubject(subject,"UTF-8");                                     //設定mail的標題物件名稱格式
message.setContent(bodycontent, "text/html; charset=UTF-8");             //設定mail的內容物件名稱格式
//-----<建立內文>---------
BodyPart messageBodyPart = new MimeBodyPart();                           //建立BodyPart物件
messageBodyPart.setContent(bodycontent,"text/html; charset=UTF-8");      //將信件內文儲存於messageBodyPart，及設定內文格式
Multipart multipart = new MimeMultipart();                     		     //建立Multipart物件
multipart.addBodyPart(messageBodyPart);                    			     //將信件內文儲存於Multipart
//-----<建立附件>---------
messageBodyPart = new MimeBodyPart();                                    //再建立一個BodyPart物件
...
multipart.addBodyPart(messageBodyPart);                                  //將附件資料儲存於Multipart
//-----<內文+附件包裹>----
