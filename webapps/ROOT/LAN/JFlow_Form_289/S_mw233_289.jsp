...
//-------------------資料庫------------------------
String SqlStr="",STime="",FDId="",Sql_FD="",SqlStrCY1="",SqlStrCY2="",SqlStrCY3="",SqlStrCY4="",SqlStrS="",SqlStrF="";
String BillNo="";
String FD_Data1="",FD_Data3="",FD_Data4="",FD_Data5="",applyman[]=null,AM="",FI_FieldCount="";
...
try
{				
	SqlStr="insert into Flow_Log (L_FormName,L_ActiveType,L_FormMemo) values ('櫃號轉移表單-JFlow_Form_289','批次轉入','開始...')";
	Data.ExecUpdateSql(Conn,SqlStr);
	%><script>alert("批次轉入開始！");</script><%
	
	//------------------櫃號組數--------------------
	SqlStrF="select * from flow_forminfo where FI_ID='JFlow_Form_289' order by FI_RecID DESC limit 1";
	ResultSet Ds=null;
	...
	TransferGroup=(FieldCount-8)/4;                          
	%><script>alert("取得櫃號組數，共<%=TransferGroup%>組！");</script><%

	//----------抓取(前1天內)已審核過的表單----------
	STime="and flow_formdata.FD_Data5<'"+EDate+"'";                                                              //櫃號轉移生效日
	SqlStr="select flow_form_rulestage.BillNo1,flow_form_rulestage.BillNo2,flow_form_rulestage.BillNo3,flow_form_rulestage.FR_FinishTime,flow_formdata.* from flow_formdata,flow_form_rulestage where flow_form_rulestage.FR_FinishTime>'"+SDate+" 00:00' and flow_form_rulestage.FR_FinishTime<'"+NowDate+" 23:59' "+Sql_FD+" and flow_form_rulestage.FR_FormAP='JFlow_Form_289' and flow_form_rulestage.FR_FinishState=1 "+STime+"and flow_formdata.FD_RecId=flow_form_rulestage.FR_DataId";		
		...
		{				
			try
			{
				...
				if(!FD_Data1.equals(""))                                                 
				{
					...
					u=0;                                                                                         //每跑完一張表單後u歸0
					//--------用for迴圈跑多組櫃號("原供應商+轉移供應商"為一組櫃號)--------
					for(int t=0;t<TransferGroup;t++)                                                             //t的參數=第幾組(0=第一組,1=第二組，類推)->參數由TransferGroup動態取值             
					{
						u++;
						Field3=String.valueOf((t*4)+3);	                                                         //原供應商欄位
						Field4=String.valueOf((t*4)+4);	                                                         //轉移供應商欄位
						Field5=String.valueOf((t*4)+5);	                                                         //生效日
						FD_Data3=DRs.getString("FD_Data"+Field3).split(":")[0].trim();                           //原供應商櫃號
						FD_Data4=DRs.getString("FD_Data"+Field4).split(":")[0].trim();                           //轉移供應商櫃號
						FD_Data5=DRs.getString("FD_Data"+Field5);                                                //生效日
						
												
						if(!FD_Data3.equals("")) 
						{
							
							SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");                           //生效日小於或等於今日才執行更新
							Date endDate=sdf.parse(JDate.ToDay()); 
							Date beginDate = sdf.parse(FD_Data5);
							...
							if(days<=0)
							{
								...
								//CHANGE_GOODS資料更新
								SqlStrCY1="update CHANGE_GOODS set COUNTER_ID='"+FD_Data4+"'where COUNTER_ID='"+FD_Data3+"'";
								stmtCY.executeUpdate(SqlStrCY1);
								out.println("CHANGE_GOODS<br>"+SqlStrCY1+"<br><br>");
										
								//GOODS_COUNTER資料更新
								SqlStrCY2="update GOODS_COUNTER set COUNTER_ID='"+FD_Data4+"'where COUNTER_ID='"+FD_Data3+"'";
								stmtCY.executeUpdate(SqlStrCY2);
								out.println("GOODS_COUNTER<br>"+SqlStrCY2+"<br><br>");
										
								//GOODS資料更新
								SqlStrCY3="update GOODS set SUPPLIER_ID='"+FD_Data4+"'where SUPPLIER_ID='"+FD_Data3+"'";
								stmtCY.executeUpdate(SqlStrCY3);
								out.println("GOODS<br>"+SqlStrCY3+"<br><br>");
										
								//GOODS_EC_PRICE資料更新
								SqlStrCY4="update GOODS_EC_PRICE set COUNTER_ID='"+FD_Data4+"'where COUNTER_ID='"+FD_Data3+"'"; 
								stmtCY.executeUpdate(SqlStrCY4);
								out.println("GOODS_EC_PRICE<br>"+SqlStrCY4+"<br><br>");
								
								Record+="【資料更新】 舊櫃號: "+FD_Data3+" 轉換成新櫃號: "+FD_Data4+" 成功！\r\n";
								Record_All+="表單:"+FDId+" 【資料更新】 舊櫃號: "+FD_Data3+" 轉換成新櫃號: "+FD_Data4+" 成功！\r\n";
...
if(catch_e!=""||catch_ex!=""||catch_d!="")
{
	SqlStr="insert into Flow_Log (L_FormName,L_ActiveType,L_FormMemo) values ('櫃號轉移表單-JFlow_Form_289','批次轉入','錯誤: Transfer.jsp  FR_DataId="+FDId+" 錯誤訊息:"+catch_e+catch_ex+catch_d+"')";
	Data.ExecUpdateSql(Conn,SqlStr);
...
{
	SqlStr="insert into Flow_Log (L_FormName,L_ActiveType,L_FormMemo) values ('櫃號轉移表單-JFlow_Form_289','批次轉入','完成.共處理 "+FCount+" 張表單,"+ICount+" 筆資料.')";
	Data.ExecUpdateSql(Conn,SqlStr);
