...
function doApply()
{
    ...
    //判斷審核人員
	/*第一關*/
	if(document.getElementById('tmp_FD_Data15').value=='')
	{
		document.getElementById('FD_Data18').innerText='';
		alert('請選擇第一關審核人員!');
		Frm.tmp_FD_Data15.focus();
		return false;
	}
	else
	{
		  
		var SurverArr=Frm.FD_Data15.value.split(',');
	...
	/*第二關*/
	if(document.getElementById('tmp_FD_Data16').value!='')
	{  
	   
		var SurverArr=Frm.FD_Data16.value.split(',');
		var ID=SurverArr[0].substring(0,5).trim();  
		if(ID!='' && ID.length==4)
		{UID="cy"+ID.trim();}
		if(ID!='' && ID.length==5)
		{UID=ID.trim()+1;}
		var Field1 = 'FD_Data19'; 
		list.push(UID);
		listfid.push(Field1);
	...
	/*第三關*/
	if(document.getElementById('tmp_FD_Data17').value!='')
	{  
	   
		var SurverArr=Frm.FD_Data17.value.split(',');
		var ID=SurverArr[0].substring(0,5).trim();  
		if(ID!='' && ID.length==4)
		{UID="cy"+ID.trim();}
		if(ID!='' && ID.length==5)
		{UID=ID.trim()+1;}
		var Field1 = 'FD_Data20'; 
		list.push(UID);
		listfid.push(Field1);
...
{   
    ...
    if(confirm('確定送出此表單嗎?'))     
    {
		...
		S1=document.getElementById('FD_Data18').innerText;
		S2=document.getElementById('FD_Data19').innerText;
		S3=document.getElementById('FD_Data20').innerText;
