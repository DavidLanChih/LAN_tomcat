...
function doApply()
{
    ...
    //判斷審核人員
	/*第一關*/
	if(document.getElementById('tmp_FD_Data18').value=='')
	{
		document.getElementById('FD_Data21').innerText='';
		alert('請選擇第一關審核人員!');
		Frm.tmp_FD_Data18.focus();
		return false;
	}
	else
	{		  
		var SurverArr=Frm.FD_Data18.value.split(',');
		var ID=SurverArr[0].substring(0,5).trim();  
		
		if(ID!='' && ID.length==4)
		{UID="cy"+ID;}
		if(ID!='' && ID.length==5)
		{UID=ID+1;}
		var Field1 = 'FD_Data21'; 
		list.push(UID);
		listfid.push(Field1);
		
		var str = Frm.FD_Data1.value.substring(0,5).trim();
		if(ID==str)
		{
			alert('項次1：主管不得審自己!');
			Frm.tmp_FD_Data1.focus();
			return false;
		}
	}
	  
	/*第二關*/
	if(document.getElementById('tmp_FD_Data19').value!='')
	{  	   
		var SurverArr=Frm.FD_Data19.value.split(',');
		var ID=SurverArr[0].substring(0,5).trim();  
		if(ID!='' && ID.length==4)
		{UID="cy"+ID.trim();}
		if(ID!='' && ID.length==5)
		{UID=ID.trim()+1;}
		var Field1 = 'FD_Data22'; 
		list.push(UID);
		listfid.push(Field1);		
	}
	
	/*第三關*/
	if(document.getElementById('tmp_FD_Data20').value!='')
	{  
		var SurverArr=Frm.FD_Data20.value.split(',');
		var ID=SurverArr[0].substring(0,5).trim();  
		if(ID!='' && ID.length==4)
		{UID="cy"+ID.trim();}
		if(ID!='' && ID.length==5)
		{UID=ID.trim()+1;}
		var Field1 = 'FD_Data23'; 
		list.push(UID);
		listfid.push(Field1);		
	}
...
{   
    ...
    if(confirm('確定送出此表單嗎?'))     
    {
		...
		S1=document.getElementById('FD_Data21').innerText;
		S2=document.getElementById('FD_Data22').innerText;
		S3=document.getElementById('FD_Data23').innerText;
